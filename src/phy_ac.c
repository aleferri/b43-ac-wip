...
/*
 * Load one of the tone tables the vendor puts in table 0x000e -- the PHY-side
 * sample-play buffer, the small CORDIC/IQ table the RX-IQ cal plays through the
 * B43_PHY_AC_SAMP_PLAY_* control block (the same "sample-play table" the wl
 * SDR path reuses to radiate; here it stays in loopback).
 *
 ...
 */
/*
 * Lunghezza del probe response: la parte fissa, che dipende dalla larghezza
 * per via degli elementi HT/VHT, piu' l'SSID.
 *
 * I numeri erano 284/285/286, che sono questi con un SSID di sette caratteri.
 * Sommare l'SSID invece di inchiodarne uno e' l'unico modo di non ritarare
 * cinque letterali alla prossima cattura: la stessa lunghezza decide anche
 * 0x001e e le celle BTL nel doppione del core.
 */
 ...
 *
 * Every value here is the same on every segment of the d6220 cold sweep, at
 * every channel and every width, and the tg789vac writes the same ones except
 * for the chain-count cells below. They are transcribed: what they are for is
 * not known.
 *
 ...
 * which falls in the KEYIDXBLOCK block and carries the coremask -- 3 on the
 * d6220, 7 on the three-chain boards; who should emit those two is not
 * settled.
 * [capture-ref: router-data/d6220/cold-sweep.zip!cold01-ch36-bw20.txt;
 *   12860-12892]
 * [capture-ref: router-data/d6220/hot-sweep.zip!segmenti/01-up-ch36-bw20.txt;
...
static void b43_phy_ac_shm_mac_config_block(struct b43_wldev *dev)
{
	...
	/*
	 * The d6220, which leaves one of its three cores unwired, writes
	 * 0x0910 and 0x08f4 and then 0x08f0 = 1; the agcombo and the tg789vac,
	 * with all three wired, skip the pair and write 0x08f0 = 2. Two
	 * boards, so "a core is unwired" and "two chains" cannot be told
	 * apart, nor "chains - 1" from other readings of 1 and 2.
	 */
...
static void b43_phy_ac_idle_tssi_meas(struct b43_wldev *dev)
{
	 ...
	 * cannot forget it.
	 *
	 * It deliberately does not hand back the RX gate: with the bracket
	 * gated at its two ends -- the arm in txpwrctrl_enable() and the close
	 * in op_switch_channel() -- there is nothing to hand back, and the
	 * counts on cold05 are exact: PHY 0x0140 five accesses, 0x0339 two,
	 * 0x02ed three, all from the channel setup and none from here.
	 */
	...
	for_each_set_bit(core, &dev->phy.ac->coremask, dev->phy.ac->num_cores) {
		 ...
		 */
		/* Prologue, once per measured core. */
		b43_phy_read(dev, B43_PHY_AC_REG_TBL_WRITE_GATE);
		...
		for_each_set_bit(c, &ac->coremask, ac->num_cores) {
			...
			b43_actab_read_bulk(dev, 0x0007, (u16)(0x017e + 0x10 * c),
					    16, 1, &tblr_dummy);
			/* pdet_range: NVRAM has no pdetrange5g on these boards
			 * (default 0); SPROM8 FEM offsets are 0xFFFF on SROM 11.
			 * Zero -> clear bits. */
		...
		for_each_set_bit(c, &ac->coremask, ac->num_cores) {
			...
			/*
			 * Ripristino di quello che st[c].r4e ha salvato all'inizio del
 ...
 * forced for the duration, and track the state. The vendor emits the pulse as
 * an atomic maskset pair -- val=<bit> mask=<bit>, then val=0 mask=<bit> -- at
 * many points of the bring-up, to kick the CCA state machine. Never as a plain
 * write: every op on 0x0001 in both cold sweeps is a MOD, 696 of 696 on the
 * d6220 and 784 of 784 on agcombo, which is why phy_maskset() is used at every
 * site.
 *
 * All thirty-eight CCA-kick pulses of cold01 are the same four operations, in
 * the same order:
 *
 *   PHY.FGC val=0x0001
 ...
 *   PHY.FGC val=0x0000
 *
 * Thirty-five are the sites below and three are channel_switch_prep()'s, and
 * there is no shape that distinguishes them. So there is one function, and
 * b43_phy_ac_reset_cca() is the exported name for it. The udelay comes with
 * the force: forcing the clock is what makes the settling time real.
 * [capture-ref: router-data/d6220/cold-sweep.zip!cold01-ch36-bw20.txt;
 ...
 * are the opcodes the sequencer steps through, distinct from the RF_SEQ_TRIG
 * trigger bits in phy_ac.h.
 *
 ...
 * below match rev3's event lists one-for-one, and the sentinel is 0x1f as in
 * rev3 (rev<3 used 0xf), which fixes the encoding.
 */
enum b43_phy_ac_rfseq_cmd {
	...
	/*
	 * SALAME: the HPC-set opcodes are carried over from the same rev3 map
	 * by analogy, not pinned by an ordering. Only the LPF (even) variants
	 * occur, next to where reset2rx loads the LPF-HPC tables; the HPF (odd)
	 * siblings 0x09/0x0b/0x0d exist in rev3 but the AC path does not use
	 * them.
	 */
	...
	/*
	 * AC-only opcodes with no N-PHY counterpart and no established meaning:
	 * named for the tables' legibility, not decoded. 0xb0-0xb2 carry a
	 * per-core second word and take 0x2b's slot in the 80 MHz variant.
	 */
 ...
 */
/*
 * Quattro campi del blocco RX gain non sono costanti ma funzione della sola
 * larghezza. Lo sweep lo dice senza margine: ognuno dei tre valori qui sotto
 * e' identico su tutti e 16 i canali a 20 MHz, su tutti e 7 quelli a 40 e su
 * tutti e 3 quelli a 80, dal 36 al 140, DFS compresi. Gli altri ventidue campi
 * del blocco non si muovono affatto.
 *
 * Sono tre valori scritti e non una legge: a differenza di 0x0738[2:0], che
 * cresce di 1 per passo, questi non seguono una progressione -- 0x0739[6:1]
 * resta fermo fra 20 e 40 e cambia a 80, 0x073a[6:5] fa il contrario.
 * Interpolare su tre punti qui vorrebbe dire inventare; a 160 MHz non c'e'
...
/*
 * Una cella della gain curve, tabella 0x0020.
 *
 * La cella e' larga 48 bit e si legge in tre parole dal data port 0x0011: la
 * prima e' il bbmult, le altre due sono i trentadue bit che il vendor
 * spacchetta nelle tre celle di coefficiente della tabella 0x0007. I campi
 * sono 8/16/8 a partire dal bit 0.
 *
 * Verificato su due celle con contenuto diverso, che danno due risultati
 * diversi: la 0x0040 legge (0x0035, 0x1300, 0xf32f) e il vendor scrive
 * (0x0000, 0x2f13, 0x00f3) con bbmult 0x35; la 0x0000 legge
 * (0x0044, 0x7f00, 0xf3ff) e scrive (0x0000, 0xff7f, 0x00f3) con bbmult 0x44.
 * Costante su tutti e 43 i segmenti, a ogni canale e a ogni larghezza.
 * [capture-ref: router-data/d6220/cold-sweep.zip!cold01-ch36-bw20.txt;
 ...
 * the cal writes back. The vendor unlocks after each read; actab_read_bulk()
 * does not, so the unlock is here.
 */
...
static void b43_phy_ac_adc_reset(struct b43_wldev *dev)
{
	...
	for_each_set_bit(c, &dev->phy.ac->coremask, num_cores) {
		...
		/* Readback gain-curve, offset 0x40.
...
static void b43_phy_ac_txpwrctrl_enable(struct b43_wldev *dev)
{
	...
	/*
	 * Arm the RX gate and drop the ADC bracket: this is the open of the
	 * bracket the idle-TSSI measurement runs inside, and it goes with the
	 * measurement. Where that does not run the vendor does not arm either
	 * -- on cold05 PHY 0x0140 has five accesses, the release above and the
	 * three of the setup, and none of the pair this would add; 0x0339 has
	 * two and 0x02ed three, same count.
	 */
	if (!b43_phy_ac_may_calibrate_tx(dev))
		return;
 ...
 */
/*
 * The middle pair belongs to the IQ/LO state and not to the AFE gain: it is
 * emitted wherever @iqlo_saved holds and nowhere else. Block E of
 * rxiqcal_finalize() always qualifies -- block D runs twenty ops earlier in
 * the same function -- so the flag is read here rather than passed, and the
 * two call sites stay one rule. The outer three are unconditional: cold05 and
 ...
 */
/*
 * Does this channel carry the radar-detection duty at all. The calibration
 * gate is this AND the check being settled; the detector poll is only this.
 */
 ...
 *
 * All of it is conditional on b43_phy_ac_may_calibrate_tx(); see the gate.
 * [capture-ref: router-data/d6220/cold-sweep.zip!cold01-ch36-bw20.txt;
 *   14968-36041]
 * [capture-ref: router-data/d6220/hot-sweep.zip!segmenti/01-up-ch36-bw20.txt;
 *   10382-28591]
...
static void b43_phy_ac_post_switch_calibrations(struct b43_wldev *dev)
{
	...
	/*
	 * The whole procedure is one calibration, and it transmits: where
	 * b43_phy_ac_may_calibrate_tx() says no, none of it runs.
	 *
	 * The gate is the function and not its phases because that is what the
	 * captures show. Against cold05 (ch52, 20 MHz) the port's surplus is a
	 * single uninterrupted run from this entry to the end of
	 * rxiqcal_finalize(): 1714 operations with not one vendor operation
	 * among them. Before it the two streams agree on the mac_enable
	 * op_channel_calibrate() owes, after it they agree again on the host
	 * flag the vendor clears next, so the run is bounded on both sides by
	 * matched operations and nothing inside it landed elsewhere.
	 *
	 * Phase by phase, where a phase has a register only it touches:
	 * post_cal_finalize_iter3() has three -- OBJ 0x00b8, PHY 0x0640 and
	 * PHY 0x0840 -- and iqcal_apply_second_stage() one, PHY 0x0211. All
	 * four are present on every one of the seven cold segments below the
	 * boundary and absent on all nineteen above it. The phases that used
	 * to carry their own gate here brought the same evidence, which is
	 * kept: table 0x000e, 8 accesses up to ch48 and 11 at 80 MHz against
	 * zero from ch52 up, for post_rxiqcal_stage2() and the DDS seeds; the
	 * IQ coefficient tables 0x42/0x62/0x82, 256 entries each below and
	 * zero above; PHY 0x0724 and 0x0736 for rxgain_config_apply(), 27
	 * accesses each at ch36 against the 9 that rx_gain_regs_program() and
	 * rxgain_perchan_tail() make above; and the twelve radio registers of
	 * radio_iqcal_config() and its teardown -- 0x0020-0x0023, 0x003a,
	 * 0x003d and the +0x200 mirrors -- 3 to 21 accesses each below and
	 * zero above, with no address shared with anything else.
	 *
	 * rxiqcal_finalize() is inside the gate and not after it: its opening
	 * write, PHY 0x040f = 0x09ff, has three accesses at ch36 and none at
	 * ch52. It also means the coefficients it saves for b43_phy_ac_bss_up()
	 * keep the values a previous bring-up left, which is what the stock
	 * driver does with them here.
	 *
	 * What separates "above 5250" from "first bring-up" is the hot sweep,
	 * where the check has long completed: there the score is flat across
...
struct b43_ac_b2j_op {
	...
	u16 val;
	/*
	 * Quanto si somma al valore per passo di banda; 0 per le voci che non
	 * si muovono. Vedi b43_phy_ac_bw_step(). Non e' un flag perche' il
	 * passo va nel peso del campo: [2:0] cresce di 1, [10:8] di 0x100,
	 * [13:11] di 0x800.
	 */
	u16 bw_step;
	/*
...
static const struct b43_ac_b2j_op b43_phy_ac_b2j_ops[] = {
	...
	/*
	 * Tre campi vicini che crescono di uno per raddoppio della banda, e li
	 * scrivo con la stessa legge perche' e' la stessa: 0x0735[10:8] fa
	 * 3/4/5 e 0x0735[13:11] 3/4/5 nel proprio peso, come 0x0738[2:0].
	 * Misurati su cold17 e cold18 a 40 MHz e cold24 a 80; i segmenti sopra
	 * i 5250 non ne hanno perche' la calibrazione che li emette li' non
	 * gira. Tre punti sono tre punti: a 160 MHz non c'e' cattura.
	 */
...
void b43_phy_ac_post_rxiqcal_stage2(struct b43_wldev *dev)
{
	...
	/*
	 * B4c, 33 ops:
	 *   - one self-contained table read of 0x000c per wired chain, at
	 *     0x63 + 4 * core, dummy readbacks. The vendor emits lock, read and unlock;
...
void b43_phy_ac_radio_chain_range_setup(struct b43_wldev *dev, bool with_tune)
{
	...
	/* 3g: two self-contained reads of a gain-curve cell, in the full
	 * variant only. */
	if (with_tune) {
		b43_phy_ac_read_chain_gaincurves(dev);
...
/*
 * Tail shared by rxgain_perchan_config(), b43_phy_ac_rxiqcal_meas_readback_kick_tail() and
 ...
 * b43_phy_ac_read_chain_gaincurves(): its three TX gain code cells into table
 * 0x0007 at 0x100 + core + 3 * i, each read back first and discarded, then
 * its bbmult into 0x63 + 4 * core and 0x73 + 4 * core, saving the cell first.
 * With 0x14 and 0x1e that is (0x0000, 0x4f7f, 0x00f3) with bbmult 0x40 and
...
void b43_phy_ac_rxiqcal_prep_second_iter(struct b43_wldev *dev)
{
	 ...
	 * entry the gate is unlocked, the previous function having ended on the
	 * unlock MOD of write_bulk_scoped(). */
	b43_phy_ac_bbmult_peek(dev);
...
void b43_phy_ac_rxiqcal_run_meas_iters(struct b43_wldev *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(iters); i++) {
		...
		/*
		 * After a chain's two iterations, the batch, if a later
...
void b43_phy_ac_rxiqcal_apply_second_stage(struct b43_wldev *dev)
{
	 ...
	 * b43_phy_ac_rxiqcal_run_meas_iters() -- reapplied at 0x60 + 4 * core
	 * of table 0x000c instead of 0x40 + 8 * core. They come out of afe_res,
	 * not from a capture.
	 */
	u16 cell[2];
	unsigned int c;
	...
	/* One pair per wired chain: TBL.RD + TBL.WR len=2, 16 ops. afe_res
	 * holds each chain's result at 2 * core, see afe_res_store(). */
	for_each_set_bit(c, &dev->phy.ac->coremask, dev->phy.ac->num_cores) {
...
void b43_phy_ac_rxgain_config_readback(struct b43_wldev *dev)
{
	...
	for_each_set_bit(core, &ac->coremask, ac->num_cores) {
		...
		/* Three gain codes of table 7, at 0x0100 + core + 3 * i: the
		 * cells rxiq_teardown_apply_defaults() writes back. */
		for (i = 0; i < 3; i++)
 ...
 *
 *   A: one self-contained bbmult table read per wired chain
...
void b43_phy_ac_rxiqcal_teardown_apply_defaults(struct b43_wldev *dev)
{
	...
	for_each_set_bit(c, &dev->phy.ac->coremask, dev->phy.ac->num_cores) {
		...
		/* Fast TBL.RD 0x0020 off=0x0040 (7 op) */
 ...
 *   four peeks per wired chain and four shared ones; see
 *   b43_phy_ac_probe_peek()
...
/*
 * The peeks at the head of every probe iteration: four per wired chain, then
 * the four shared ones. Core 2 reads its four in a different order from cores
 * 0 and 1, on the agcombo and the tg789vac alike; the d6220 does not wire it
...
/*
 * Take the current value of 0x0520[3:2] and step it to the next wired chain:
 * the field selects a chain, and the vendor walks them in turn -- 0, 1 on the
 * d6220, 0, 1, 2 on the tg789vac. The counter lives in ac->probe_mode because
 * the stock driver does not reset it between one probe block and the next:
 * the walk runs continuously across the whole phase.
 */
 ...
 */
/*
 * Le letture di fase, e con @arm_tone la coppia del tone generator.
 *
 * Estratta da b43_phy_ac_wd_sample_phase_opt() perche' all'ingresso della fase
 * probe non sta al suo posto: la cattura la mette fra le quattro celle sparse
 * della spazzata e il cambio di modo, mentre a regime viene dopo entrambi.
...
/* Il cambio di modo che chiude il giro, estratto per la stessa ragione. */
...
void b43_phy_ac_rxiqcal_finalize(struct b43_wldev *dev)
{
	...
	/*
	 * Block B: one self-contained two-word table write to id 0x000c per
	 * wired chain. Reapplies the AFE cal's pass-1 results, the afe_res_cal
	 * snapshot of the 0x?056 iterations, at 0x60 + 4 * core. The d6220
	 * attach capture shows (0x0066, 0x000e) and (0x0027, 0x0003).
	 */
	{
		unsigned int c;
	 ...
	 */
	{
		...
		/* The write-back in b43_phy_ac_down() has something to say. */
	 ...
	 * MAC suspended, and what follows them does not. It belongs here and
	 * not to the tail below, which on a channel where the calibration did
	 * not run is entered with the MAC already enabled by
	 * op_channel_calibrate().
	 */
	b43_mac_enable(dev);
}
...
static void b43_phy_ac_cac_poll(struct b43_wldev *dev, unsigned int turns)
{
	...
	/*
	 * The duty alone, not the calibration gate. Those two were the same
	 * test for as long as every captured segment on a radar channel had
	 * the check outstanding from the arm to the end, so no capture could
	 * separate them. The DFS recapture does: on cold05 the detector is
	 * read 706 times between +10.7 s and +147.5 s, and 305 of those fall
	 * after +73.6 s, which is where the check settles and the tone
	 * generator starts. The poll is in-service monitoring and outlives the
	 * check; the calibrations are what waits for it.
	 */
	if (!b43_phy_ac_chan_has_radar_duty(dev))
		return;
...
/*
 * Il latch della finestra statistiche e il blocco E: le soglie CRS e il banco
 * 0x0910, che dipendono dal campione appena latchato.
 */
/*
 * Il blocco E: le soglie CRS e il banco 0x0910, che dipendono dal campione
 * che il latch ha appena preso. Separato dal latch perche' sopra i 5250 i due
 * non stanno insieme -- vedi il richiamo dentro il giro del watchdog.
 */
static void b43_phy_ac_crs_block_e(struct b43_wldev *dev)
{
	b43_mac_suspend(dev);
	 ...
	 * statistiche del tick, o i turni di CAC -- gira a MAC attivo, ed e'
	 * chi lo usa a sospenderlo e riabilitarlo. Su cold01 wl emette qui una
	 * sola abilitazione; una coppia enable+suspend lascerebbe il MAC
	 * sospeso e sfaserebbe di un'op tutti e venti i tick.
	 */
	b43_mac_enable(dev);
}
 ...
 * probe and watchdog ticks, and the closing per-chain and power-target writes.
 *
 * Separate from b43_phy_ac_rxiqcal_finalize() because the two answer to
 * different conditions. The calibration and its teardown do not run where
 * b43_phy_ac_may_calibrate_tx() says no; this does, and the captures above the
 * boundary are almost entirely this. On cold05 the vendor goes from the
...
static void b43_phy_ac_post_bringup_tail(struct b43_wldev *dev)
{
	...
	/*
	 * Fra la host-flag clear e le ricariche del beacon, dove la cattura la
	 * mette.
	 */
...
static void b43_phy_ac_down(struct b43_wldev *dev)
{
	 ...
	 *
	 * Only where block D has run: see @iqlo_saved in phy_ac.h for what
	 * separates that from b43_phy_ac_may_calibrate_tx(), and for why the
	 * cold sweep alone cannot.
	 *
	 * The solver runs again over the same accumulators, so it writes the
...
static int b43_phy_ac_op_switch_channel(struct b43_wldev *dev, unsigned int new_channel)
{
	 ...
	 */
	{
		 ...
		 * emits before table 0x0b, on the d6220 and the tg789vac alike.
		 * The agcombo's older driver has no op on table 0x15 at all.
		 */
		for (core = 0; core < dev->phy.ac->num_cores; core++)
	...
	if (b43_phy_ac_may_calibrate_tx(dev)) {
		 ...
		 * gap is ~35 ms and this waits 35 us: idle_tssi_meas() already
		 * polls its own busy bit, so the gap is read as the stock
		 * driver's scheduling.
		 * SALAME: if the base index comes out wrong on hardware,
		 * mdelay(35).
		 */
		udelay(35);

		/*
		 * The close of the bracket txpwrctrl_enable() opened, and the
		 * last op of the measurement: 0x0339 back to 0x0fff. It goes
		 * with the measurement for the same reason the arm does, and
		 * the count says so -- two accesses on cold05, both from the
		 * channel setup, none here.
		 */
		b43_phy_write(dev, 0x0339, 0x0fff);
	}
