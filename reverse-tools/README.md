...
- **anchors.py** — i riferimenti `#NNNN` nei commenti: `resolve` dice quali
  ...
  `write` invece **scrive**: genera nel commento di ogni funzione istrumentata
  il marker `[capture-ref: <cattura>; <da>-<a>, ...]`, sostituendo quello che
  c'e'. E' la risposta all'ancoraggio che invecchia: un marker non si mantiene
  a mano, si rigenera dagli stessi span che `span` controlla, quindi un hook
  che si sposta o una cattura sostituita si presentano come un diff invece di
  invecchiare in silenzio. Le run vengono da `capture-refs.conf`, cosi' la
  rigenerazione e' riproducibile. `--remove` li toglie tutti, `--dry-run` dice
  cosa cambierebbe. La granularita' e' quella dell'istrumentazione: una per
  `B43_AC_FN()`, piu' una per ogni `B43_AC_BLOCK("nome")` dove una sezione di
  una funzione lunga vale la pena localizzarla da sola. Va scritta la lista
  **completa** degli intervalli, non il loro inviluppo: una funzione chiamata
  35 volte copre 35 tratti di cattura e il min-max di quelli e' un intervallo
  che non copre.
