...
/*
 * Block markers (see B43_AC_BLOCK in phy_ac.h). Same stream, same gate, a
 * different tag so the tools can tell a named section from a function. Point
 * marker: there is no closing counterpart to emit, the reader closes a block
 * where the next one opens or where the function leaves.
 */
