...
4. Cattura **attach del DSL** per sbloccare le 8 fasi non confrontabili + init/PLL da freddo. `reverse-tools/cold_capture.sh` la da': l'ordine delle notifiche di modulo in 2.6.30 e' stato controllato su `kernel/module.c` di quel kernel ed e' lo stesso del 3.4 (vedi `reverse-tools/wl-diag/README.md`), e il tracer per il 2.6.30 ora riarma al COMING e disarma al GOING. Il `rmmod wl` che lo spazio utente del vendor fa al rescan non e' piu' un ostacolo, dato che il tracer non tiene un riferimento sul bersaglio.
